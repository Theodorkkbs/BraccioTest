
/*
  simpleMovements.ino

 This  sketch simpleMovements shows how they move each servo motor of Braccio

 Created on 18 Nov 2015
 by Andrea Martino

 This example is in the public domain.
 */

#include <Braccio.h>
#include <Servo.h>
#include <SoftwareSerial.h>
/* The circuit:
 * RX is digital pin 2 (connect to TX of other device)
 * TX is digital pin 3 (connect to RX of other device)
 * 
  */
SoftwareSerial hc06(2,3); //declaring pins


Servo base;
Servo shoulder;
Servo elbow;
Servo wrist_rot;
Servo wrist_ver;
Servo gripper;

String msg="";

void setup() {
  Initialization functions and set up the initial position for Braccio
  All the servo motors will be positioned in the "safety" position:
  Base (M1):90 degrees
  Shoulder (M2): 45 degrees
  Elbow (M3): 180 degrees
  Wrist vertical (M4): 180 degrees
  Wrist rotation (M5): 90 degrees
  gripper (M6): 10 degrees
  Braccio.begin();
  hc06.begin();
  hco6.println("Bluetooth On  ..");

}

void loop() {
   /*
   Step Delay: a milliseconds delay between the movement of each servo.  Allowed values from 10 to 30 msec.
   M1=base degrees. Allowed values from 0 to 180 degrees
   M2=shoulder degrees. Allowed values from 15 to 165 degrees
   M3=elbow degrees. Allowed values from 0 to 180 degrees
   M4=wrist vertical degrees. Allowed values from 0 to 180 degrees
   M5=wrist rotation degrees. Allowed values from 0 to 180 degrees
   M6=gripper degrees. Allowed values from 10 to 73 degrees. 10: the toungue is open, 73: the gripper is closed.
  */
  
 /*                      //(step delay, M1, M2, M3, M4, M5, M6);
  Braccio.ServoMovement(20,           0,  15, 180, 170, 0,  73);  
*/
  if (hc06.available())
  {
    msg=hc06.read();
  }
 bool quit=false;
  while(!quit){
  switch(msg){
    case"gripLeft":
      Braccio.ServoMovement(20,           90,  45, 180, 180, 90,  10);  
      break;
       case"gripRight":
      Braccio.ServoMovement(20,           90,  45, 180, 180, 90,  73);  
      break;
       case"baseLeft":
      Braccio.ServoMovement(20,           0,  45, 180, 180, 90,  10);  
      break;
       case"baseRight":
      Braccio.ServoMovement(20,           180,  45, 180, 180, 90,  10);  
      break;
       case"elbowLeft":
      Braccio.ServoMovement(20,           90,  45, 0, 180, 90,  10);  
      break;
       case"elbowRight":
      Braccio.ServoMovement(20,           90,  45, 180, 180, 90,  10);  
      break;
       case"shoulderLeft":
      Braccio.ServoMovement(20,           90,  15, 180, 180, 90,  10);  
      break;
       case"shoulderRight":
      Braccio.ServoMovement(20,           90,  165, 180, 180, 90,  10);  
      break;
       case"wristRotLeft":
      Braccio.ServoMovement(20,           90,  45, 180, 180, 0,  10);  
      break;
       case"wristRotRight":
      Braccio.ServoMovement(20,           90,  45, 180, 180, 180,  10);  
      break;
        case"wristVertLeft":
      Braccio.ServoMovement(20,           90,  45, 180, 0, 180,  10);  
      break;
        case"wristVertRight":
      Braccio.ServoMovement(20,           90,  45, 180, 180, 180,  10);  
      break;
        case"quit":
        quit=true;
      break;
      
  }
  }
  //Wait 1 second
  delay(1000);

}
